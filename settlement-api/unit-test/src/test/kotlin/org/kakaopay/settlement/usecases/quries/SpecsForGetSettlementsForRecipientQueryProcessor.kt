...
class SpecsForGetSettlementsForRecipientQueryProcessor(
    @Autowired val settlementRepository: SettlementRepository
) {

    @ParameterizedTest
    @AutoSource
    fun sut_returns_settlements_with_recipientId_of_query_correctly(
        settlement: Settlement,
        @Min(1) @Max(10) size: Int
    ) {
        //Arrange
        arrangeSettlements(settlement, size)
        val sut = GetSettlementsForRecipientQueryProcessor(
            settlementRepository
        )
        val query = GetSettlementForRecipientQuery(
            settlement.recipients.first().userId
        )
